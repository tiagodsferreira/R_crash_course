---
title: "Análise de dados longitudinais: Parte I - Breve introdução ao R"
...
- Ambiente computacional e linguagem de programação  
- Ferramenta para análise estatística e visualização de dados  
...
|Gráficos profissionais                   |Gráficos básicos                           |
|Procedimentos estatísticos ilimitados    |Procedimentos estatísticos limitados       |
|Programação aberta e flexível            |Programação fechada                        |
...
A instalação básica do R incluí um conjunto de recurso que permitem:  
- Criar diferentes tipos de estruturas de dados  
- Editar e gerir dados  
- Analisar e visualizar dados    


*Estas funcionalidades são facilmente ampliadas pela instalação de pacotes desenvolvidos pela comunidade de utilizadores do R. Estes pacotes estão disponíveis para utilização livre em [CRAN](https://www.r-project.org/).* 


## O RStudio (IDE)  
Painéis do RStudio  
- Código fonte (**File -> New File -> R Script**)  
- Consola do R  
- Ambiente e Histórico  
- Ficheiros, gráficos, pacotes e ajuda  

```{r, out.width = "600px", echo = FALSE}
knitr::include_graphics("G:\\My Drive\\FPCEUP\\R trainning\\GitRepo\\R crash course\\R crash course-Figures\\RStudio_panels.jpg")
```

## Razões para utilizar o RStudio  
O RStudio é um ambiente de desenvolvimento integrado (IDE) que torna mais simples a utilização do R:  
...
## Comunicação com o R - criar objetos
A capacidade de formar e manipular objetos é uma das caraterísticas centrais do R   
...
## Comunicação com o R - criar (mais) objetos
A lista de objetos que podemos formar no R é grande  
  - Desde simples conjuntos numéricos até listas de grandes bases de dados
...
A interação com o R é feita através de uma linguagem de programação ou sintaxe  
  
- Para além de criar objetos (<-) o R dispõe de vários operadores para manipular objetos 
  
- Os operadores mais simples são:   
  - Aritméticos  
  - Relacionais  
  - Booleanos

*Na verdade, estes operadores são funções muito simples, que não precisam de mais de um ou dois argumentos para realizar um determinado procedimento*
...
- Habitualmente, quando falamos em bases de dados, pensamos em estruturas tabulares com linhas e colunas  
...
*Geralmente, as linhas correspondem a observações e as colunas a variáveis*  
...
**IMP. Não é possível ter mais do que um tipo de dados em vetores e matrizes!!!** 
...
knitr::include_graphics("G:\\My Drive\\FPCEUP\\R trainning\\GitRepo\\R crash course\\R crash course-Figures\\Rimport.JPG")
```
...
## Funções Básicas - Sumário de dados (cont.)
```{r, echo = TRUE}
...
## Pacotes R - Instalar, carregar, atualizar, descarregar e desinstalar  
Um pacote no R consiste numa coleção de funções, dados e código compilado - Existe um grande número de [pacotes para R]("http://cran.stat.ucla.edu/web/packages/available_packages_by_name.html")  
...
```{r, echo=TRUE}
bechdel <- fread("G:\\My Drive\\FPCEUP\\R trainning\\GitRepo\\R crash course\\R_crash_course\\R crash course_Data\\bechdel.csv")
names(bechdel)
head(bechdel,3)
```
