...
bechdel <-
  read.table(
    "G:\\My Drive\\FPCEUP\\R trainning\\GitRepo\\R crash course\\R_crash_course\\R crash course-Data\\bechdel.csv",
    header = TRUE,
    sep = ";",
    quote = "\"", 
    dec = ".",
...
?read.spss
bechdel <- read.spss("G:\\My Drive\\FPCEUP\\R trainning\\GitRepo\\R crash course\\R_crash_course\\R crash course-Data\\bechdel.sav", 
                     to.data.frame = TRUE)
str(bechdel)
dim(bechdel)
bechdel[1234, ]
bechdel <- spss.get("G:\\My Drive\\FPCEUP\\R trainning\\GitRepo\\R crash course\\R_crash_course\\R crash course-Data\\bechdel.sav")
str(bechdel)
head(bechdel, 3)
